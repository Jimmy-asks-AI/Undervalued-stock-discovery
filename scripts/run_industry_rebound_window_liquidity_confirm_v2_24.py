...
DEFAULT_POLICY = ROOT / "configs" / "rebound_window_liquidity_confirm_policy_v2_24.json"
V20_SCRIPT = ROOT / "scripts" / "run_industry_rebound_window_online_state_machine_v2_20.py"
VERSION = "2.24.0"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run V2.24 liquidity-confirmed rebound-window audit.")
    parser.add_argument("--policy", default=str(DEFAULT_POLICY), help="V2.24 policy JSON.")
    ...
    amount_matrix = load_amount_matrix(ROOT / policy["history_dir"])
    features = v20.build_daily_features(close_matrix, source_policy)
    features = add_liquidity_features(features, amount_matrix)
    ...
    panel.to_csv(debug_dir / "liquidity_state_panel.csv", index=False, encoding="utf-8-sig")
    label_summary.to_csv(debug_dir / "liquidity_state_summary.csv", index=False, encoding="utf-8-sig")
    ...
    print("V2.24成交额流动性确认反弹窗口审计完成")
    print(f"日频流动性标签面板行数={run_summary['state_label_panel_count']}")
...
def add_liquidity_features(features: pd.DataFrame, amount_matrix: pd.DataFrame) -> pd.DataFrame:
    if features.empty:
        return features
    output = features.copy()
    output["trade_date"] = pd.to_datetime(output["trade_date"], errors="coerce")
    if amount_matrix.empty:
        output["market_amount_total"] = math.nan
        output["market_amount_5d_vs_20d"] = math.nan
        output["market_amount_20d_vs_120d"] = math.nan
        output["market_amount_zscore_20d"] = math.nan
        output["liquidity_repair_5d"] = math.nan
    ...
    liq["market_amount_avg_5d"] = pd.to_numeric(liq["market_amount_total"], errors="coerce").rolling(5, min_periods=3).mean()
    liq["market_amount_avg_20d"] = pd.to_numeric(liq["market_amount_total"], errors="coerce").rolling(20, min_periods=10).mean()
    liq["market_amount_avg_120d"] = pd.to_numeric(liq["market_amount_total"], errors="coerce").rolling(120, min_periods=60).mean()
    liq["market_amount_5d_vs_20d"] = liq["market_amount_avg_5d"] / liq["market_amount_avg_20d"]
    liq["market_amount_20d_vs_120d"] = liq["market_amount_avg_20d"] / liq["market_amount_avg_120d"]
    amount_std20 = pd.to_numeric(liq["market_amount_total"], errors="coerce").rolling(20, min_periods=10).std()
    liq["market_amount_zscore_20d"] = (liq["market_amount_total"] - liq["market_amount_avg_20d"]) / amount_std20
    liq["liquidity_repair_5d"] = liq["market_amount_5d_vs_20d"] - liq["market_amount_5d_vs_20d"].shift(5)
    keep = [
        "trade_date",
        "market_amount_total",
        "amount_industry_count",
        ...
        rows.append(
            {
                ...
                "market_amount_20d_vs_120d": row.get("market_amount_20d_vs_120d", math.nan),
                "market_amount_zscore_20d": row.get("market_amount_zscore_20d", math.nan),
                "liquidity_repair_5d": row.get("liquidity_repair_5d", math.nan),
            }
        )
        last_i = pos
...
def build_leakage_audit(policy: dict[str, Any], source_policy: dict[str, Any], panel: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "audit_item": "predeclared_liquidity_state_labels",
                "status": "pass",
                "evidence": f"state_labels={len(policy['state_labels'])}; horizons={policy['horizons']}",
                "action": "V2.24只审计配置中预先声明的市场状态和成交额流动性条件。",
            },
            {
            ...
            {
                "audit_item": "liquidity_features_pit_boundary",
                "status": "pass" if panel["market_amount_total"].notna().any() else "fail",
                "evidence": f"amount_rows={int(panel['market_amount_total'].notna().sum())}; liquidity_proxy={policy['liquidity_proxy']['source_field']}",
                "action": "成交额流动性代理只使用触发日及以前的本地行业成交额。",
            },
            {
...
def build_optimization_notes(top: pd.DataFrame, summary: pd.DataFrame, rolling: pd.DataFrame, annual: pd.DataFrame, policy: dict[str, Any]) -> dict[str, Any]:
    if top.empty:
        return {"main_diagnosis": "V2.24没有可排序状态。", "next_iterations": ["检查日频流动性状态面板。"]}
    best = top.iloc[0].to_dict()
    candidates = summary[summary["state_status"] == "市场状态候选"] if not summary.empty else pd.DataFrame()
    notes: list[str] = []
    if candidates.empty:
        notes.append("V2.24没有发现可泛化的成交额流动性确认候选。")
    else:
        notes.append("V2.24发现成交额流动性确认候选，但仍必须保持research_only。")
    notes.append(
    ...
    if str(best.get("state_status")) == "样本不足":
        notes.append("加入成交额确认后仍容易样本不足；这说明流动性过滤可能提升纯度，但会进一步牺牲可验证样本。")
    ...
    return {
        ...
        "recommended_next_direction": "本轮完成后先停止继续调参；应把V2.24作为证据边界，只有接入更长历史或更独立外生风险偏好数据后再重启迭代。",
    }
...
def build_run_summary(policy: dict[str, Any], panel: pd.DataFrame, summary: pd.DataFrame, top: pd.DataFrame, leakage: pd.DataFrame, notes: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "liquidity_state_panel_count": int(len(panel)),
...
def render_report(summary: dict[str, Any], top: pd.DataFrame, events: pd.DataFrame, rolling: pd.DataFrame, annual: pd.DataFrame, leakage: pd.DataFrame, notes: dict[str, Any], policy: dict[str, Any]) -> str:
    lines = ["# V2.24 成交额流动性确认反弹窗口审计报告", "", f"版本：{VERSION}", "", "## 研究结论", ""]
    lines.append("V2.24 在 V2.23 市场状态标签基础上加入本地行业成交额流动性代理，审计成交额确认是否能降低坏窗口并减少年份集中。")
    lines += [
        "",
        f"- 日频流动性标签面板行数：{summary['liquidity_state_panel_count']}",
    ...
    lines.extend(table_or_empty(best_events.head(20), {
        ...
        "market_amount_5d_vs_20d": "5/20日成交额",
        "market_amount_20d_vs_120d": "20/120日成交额",
        "market_amount_zscore_20d": "成交额Z分",
        "liquidity_repair_5d": "流动性修复",
    }, {
        "event_return",
        "market_stress_score",
    ...
    lines += [
        ...
        "## 输出文件说明",
        "",
        "- `report.md`：中文成交额流动性确认审计报告，优先打开。",
        "- `top_candidates.csv`：流动性状态标签排序；不是交易信号。",
        "- `run_summary.json`：机器可读运行摘要。",
        "- `debug/`：流动性标签面板、状态汇总、非重叠事件、样本外、滚动稳定性、年度分布、审计和冻结策略。",
        "",
