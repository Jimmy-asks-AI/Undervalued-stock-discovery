...
DEFAULT_OUTPUT = ROOT / "outputs" / "industry_pressure_quality_v2_3"
VERSION = "2.3.0"
...
STRATEGIES = [
    ...
    StrategySpec(
        "pressure_quality_reversal",
        "V2.3：压力质量反转",
        "pressure_quality_score",
        "eligible_pressure_quality",
        "conditional_only_signal",
        "市场压力较高时只保留价格质量代理较好的超跌行业。",
    ),
    StrategySpec(
        "pressure_quality_stabilized_reversal",
        "V2.3：压力质量企稳",
        "pressure_quality_score",
        "eligible_pressure_quality_stabilized",
        "conditional_only_signal",
        "在压力质量过滤基础上要求短期修复确认。",
    ),
    StrategySpec(
        "pressure_quality_liquid_reversal",
        "V2.3：压力质量流动性",
        "pressure_quality_score",
        "eligible_pressure_quality_liquid",
        "conditional_only_signal",
        "在压力反转中优先选择流动性较好的行业指数。",
    ),
    StrategySpec(
        "pressure_quality_defensive_reversal",
        "V2.3：压力质量低波",
        "pressure_quality_score",
        "eligible_pressure_quality_defensive",
        "conditional_only_signal",
        "在压力反转中优先选择低波动且回撤伤害较小的行业。",
    ),
]


def main() -> None:
    parser = argparse.ArgumentParser(description="Run V2.3 pressure quality reversal validation.")
    ...
    signal_panel = build_pressure_quality_signal_panel(
        features,
    ...
    quality_filter_impact = compute_quality_filter_impact(parameter_sensitivity)
    ...
    signal_panel.to_csv(debug_dir / "pressure_quality_signal_panel.csv", index=False, encoding="utf-8-sig")
    ...
    quality_filter_impact.to_csv(debug_dir / "quality_filter_impact.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "research_boundary": "V2.3 只研究申万行业指数压力环境下的价格质量过滤；不做个股筛选，不生成交易指令，不使用当前估值回填历史。",
        ...
        "v2_3_strategy_count": int(sum(strategy.strategy_id.startswith("pressure_quality") for strategy in STRATEGIES)),
        "stress_threshold": args.stress_threshold,
        "extreme_stress_threshold": args.extreme_stress_threshold,
    ...
    (output_dir / "report.md").write_text(
        render_report(
            ...
            quality_filter_impact=quality_filter_impact,
            bootstrap=bootstrap,
...
def build_pressure_quality_signal_panel(features: pd.DataFrame, stress_threshold: float, extreme_stress_threshold: float) -> pd.DataFrame:
    frame = features.copy()
    ...
    frame["relative_recovery_rank"] = frame.groupby("trade_date")["relative_return_20d"].rank(pct=True, method="average")
    ...
    frame["liquidity_quality_score"] = frame.groupby("trade_date")["liquidity_quality_raw"].rank(pct=True, method="average")
    frame["low_volatility_quality_score"] = frame.groupby("trade_date")["volatility_60d"].rank(pct=True, ascending=False, method="average")
    frame["drawdown_quality_score"] = frame.groupby("trade_date")["drawdown_252d"].rank(pct=True, method="average")
    ...
    frame["relative_trend_quality_score"] = frame.groupby("trade_date")["relative_trend_quality_raw"].rank(pct=True, method="average")
    ...
    frame["price_quality_composite"] = frame.groupby("trade_date")["price_quality_composite_raw"].rank(pct=True, method="average")
    ...
    frame["pressure_quality_score"] = frame.groupby("trade_date")["pressure_quality_score_raw"].rank(pct=True, method="average")
    ...
    frame["quality_gate"] = frame["price_quality_composite"] >= 0.50
    frame["quality_middle_tail_gate"] = frame["stabilized_oversold_signal"].between(0.45, 0.95, inclusive="both")
    frame["eligible_pressure_quality"] = (
        frame["pressure_gate"]
        & frame["not_momentum_trap"]
        & frame["quality_gate"]
        & frame["quality_middle_tail_gate"]
    )
    frame["eligible_pressure_quality_stabilized"] = (
        frame["eligible_pressure_quality"]
        & frame["stabilized_gate"]
        & (frame["recovery_quality_score"] >= 0.50)
    )
    frame["eligible_pressure_quality_liquid"] = (
        frame["pressure_gate"]
        & frame["not_momentum_trap"]
        & (frame["liquidity_quality_score"] >= 0.50)
        & (frame["price_quality_composite"] >= 0.45)
        & (frame["stabilized_oversold_signal"] >= 0.45)
    )
    frame["eligible_pressure_quality_defensive"] = (
        frame["pressure_gate"]
        & frame["not_momentum_trap"]
        & (frame["low_volatility_quality_score"] >= 0.50)
                    ...
                    rows.append(
                        {
                            ...
                            "avg_price_quality_composite": float(selected["price_quality_composite"].mean())
                            if "price_quality_composite" in selected.columns
                            else math.nan,
                            "avg_liquidity_quality_score": float(selected["liquidity_quality_score"].mean())
                            if "liquidity_quality_score" in selected.columns
                            else math.nan,
                            "avg_low_volatility_quality_score": float(selected["low_volatility_quality_score"].mean())
                            if "low_volatility_quality_score" in selected.columns
                            else math.nan,
                            "avg_recovery_quality_score": float(selected["recovery_quality_score"].mean())
                            if "recovery_quality_score" in selected.columns
                            else math.nan,
        ...
        rows.append(
            {
                ...
                "rejection_reasons": "；".join(reasons) if reasons else "通过 V2.3 候选门槛",
            }
        )
    return pd.DataFrame(rows)
        ...
        rows.append(
            {
                ...
                "平均质量分": fmt_pct(row.get("avg_price_quality_composite", math.nan)),
                "平均流动性分": fmt_pct(row.get("avg_liquidity_quality_score", math.nan)),
                "平均低波分": fmt_pct(row.get("avg_low_volatility_quality_score", math.nan)),
                "平均修复分": fmt_pct(row.get("avg_recovery_quality_score", math.nan)),
                "样本数": int(row["samples"]),
...
def compute_quality_filter_impact(parameter_sensitivity: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    if parameter_sensitivity.empty:
        return pd.DataFrame(rows)
    comparisons = {
        "pressure_quality_reversal": "pressure_reversal",
        "pressure_quality_stabilized_reversal": "pressure_stabilized_reversal",
        "pressure_quality_liquid_reversal": "pressure_reversal",
        "pressure_quality_defensive_reversal": "pressure_reversal",
    }
    frame = parameter_sensitivity.copy()
    for quality_strategy, base_strategy in comparisons.items():
        quality_rows = frame[frame["strategy_id"] == quality_strategy]
        base_rows = frame[frame["strategy_id"] == base_strategy]
        for quality in quality_rows.to_dict("records"):
            base = base_rows[
                (base_rows["top_n"] == quality["top_n"])
                & (base_rows["horizon"] == quality["horizon"])
            ]
            if base.empty:
                continue
            ...
            rows.append(
                {
                    "quality_strategy_id": quality_strategy,
                    "quality_strategy_zh": quality["strategy_zh"],
                    "base_strategy_id": base_strategy,
                    "base_strategy_zh": base_row["strategy_zh"],
                    "top_n": int(quality["top_n"]),
                    "horizon": int(quality["horizon"]),
                    "delta_mean_relative_return": safe_number(quality["mean_relative_return"])
                    - safe_number(base_row["mean_relative_return"]),
                    "delta_oos_mean_relative_return": safe_number(quality["oos_mean_relative_return"])
                    - safe_number(base_row["oos_mean_relative_return"]),
                    "delta_nonoverlap_mean_relative_return": safe_number(quality["nonoverlap_mean_relative_return"])
                    - safe_number(base_row["nonoverlap_mean_relative_return"]),
                    "delta_bootstrap_ci_5": safe_number(quality["bootstrap_ci_5"]) - safe_number(base_row["bootstrap_ci_5"]),
                    "delta_relative_final_nav": safe_number(quality["relative_final_nav"]) - safe_number(base_row["relative_final_nav"]),
                    "delta_samples": int(quality["samples"]) - int(base_row["samples"]),
                    "delta_nonoverlap_samples": int(quality["nonoverlap_samples"]) - int(base_row["nonoverlap_samples"]),
                    "delta_nav_daily_rows": int(quality["nav_daily_rows"]) - int(base_row["nav_daily_rows"]),
                    "quality_sample_strength": quality["sample_strength"],
                    "base_sample_strength": base_row["sample_strength"],
                }
            )
    if not rows:
        return pd.DataFrame(rows)
    return pd.DataFrame(rows).sort_values(
        ["delta_mean_relative_return", "delta_oos_mean_relative_return", "delta_nonoverlap_mean_relative_return"],
        ascending=False,
    )
...
def render_report(
    *,
    ...
    quality_filter_impact: pd.DataFrame,
    bootstrap: pd.DataFrame,
    ...
    lines = [
        ...
        f"- V2.3 质量过滤策略数：{summary['v2_3_strategy_count']}",
        "",
        "V2.3 在 V2.2 压力反转框架上加入价格质量代理过滤。这里的质量不是基本面质量，而是 PIT 可得的流动性、低波动、相对趋势韧性、回撤伤害和短期修复。当前严格门槛下，若候选信号仍为 0，则不能升级为 alpha；只能保留为条件观察或拒绝基线。",
        "",
        "## 策略排序",
        "",
    ]
    ...
    lines.extend(["", "## 质量过滤增量", ""])
    if quality_filter_impact.empty:
        lines.append("未生成质量过滤增量对比。")
    else:
        display = quality_filter_impact.head(12).copy()
        display = display[
            [
                "quality_strategy_zh",
                "base_strategy_zh",
                "top_n",
                "horizon",
                "delta_mean_relative_return",
                "delta_oos_mean_relative_return",
                "delta_nonoverlap_mean_relative_return",
                "delta_bootstrap_ci_5",
                "delta_relative_final_nav",
                "delta_samples",
                "quality_sample_strength",
                "base_sample_strength",
            ]
        ].rename(
            columns={
                "quality_strategy_zh": "质量策略",
                "base_strategy_zh": "对照策略",
                "top_n": "TopN",
                "horizon": "持有期",
                "delta_mean_relative_return": "全样本增量",
                "delta_oos_mean_relative_return": "样本外增量",
                "delta_nonoverlap_mean_relative_return": "非重叠增量",
                "delta_bootstrap_ci_5": "Bootstrap下沿增量",
                "delta_relative_final_nav": "相对净值增量",
                "delta_samples": "样本数变化",
                "quality_sample_strength": "质量样本强度",
                "base_sample_strength": "对照样本强度",
            }
        )
        for col in ["全样本增量", "样本外增量", "非重叠增量", "Bootstrap下沿增量", "质量样本强度", "对照样本强度"]:
            display[col] = display[col].map(fmt_pct)
        display["相对净值增量"] = display["相对净值增量"].map(lambda value: fmt_float(value, 3))
    ...
    lines.extend(
        [
            ...
            "## 复现文件",
            "",
            "- `debug/pressure_quality_signal_panel.csv`",
            ...
            "- `debug/quality_filter_impact.csv`",
