...
try:
    from valuation_pit_contract import ValuationPITContractError, attach_pit_valuation_asof
except ModuleNotFoundError:  # package-style imports in tests and audits
    from scripts.valuation_pit_contract import ValuationPITContractError, attach_pit_valuation_asof
...
OUT = ROOT / "outputs" / "audit" / "rebound_leader_pit_valuation_audit_v5_11"
DEBUG = OUT / "debug"
FEATURES = ["low_pb_rank", "low_pe_rank", "dividend_yield_rank", "beta_low_pb_score"]
TOP_N = 5


def main() -> None:
    parser = argparse.ArgumentParser(description="V5.11 PIT valuation audit for rebound leader rules.")
    ...
    try:
        panel = attach_valuation_features()
...
def attach_valuation_features() -> pd.DataFrame:
    ...
    for _, event in merged.groupby(["signal_date", "entry_date", "exit_date"], sort=False):
        event = event.copy()
        event["low_pb_rank"] = pd.to_numeric(event["pb"], errors="coerce").rank(pct=True, ascending=False)
        event["low_pe_rank"] = pd.to_numeric(event["pe"], errors="coerce").rank(pct=True, ascending=False)
        event["dividend_yield_rank"] = pd.to_numeric(event["dividend_yield"], errors="coerce").rank(pct=True)
        event["beta_low_pb_score"] = 0.70 * event["beta_120_rank"] + 0.30 * event["low_pb_rank"]
        pieces.append(event)
...
def build_summary(results: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "version": "5.11.0",
        "policy_id": "rebound_leader_pit_valuation_audit_v5_11",
        ...
        "best_status": "pass_pit_valuation_leader_gate" if passed else "research_only_no_pit_valuation_alpha",
        "final_verdict": "V5.11 PIT 估值特征未通过完整强行业门槛，不能声称目标完成。" if not passed else "V5.11 PIT 估值特征通过强行业门槛，但仍需前推验证。",
    }
...
def write_outputs(summary: dict[str, Any], panel: pd.DataFrame, events: pd.DataFrame, results: pd.DataFrame) -> None:
    ...
    panel.to_csv(DEBUG / "pit_valuation_opportunity_set.csv", index=False, encoding="utf-8-sig")
    events.to_csv(DEBUG / "pit_valuation_event_panel.csv", index=False, encoding="utf-8-sig")
    results.to_csv(DEBUG / "pit_valuation_results.csv", index=False, encoding="utf-8-sig")
...
def render_report(summary: dict[str, Any], results: pd.DataFrame) -> str:
    return "\n".join([
        ...
        "",
        "边界：V5.11 只接收通过严格 PIT 合同的 PE/PB/股息率，并按 available_date 向后 as-of 关联；缺少真实发布时间、版本或修订证据时整轮失败关闭。",
    ])
...
def write_blocked_outputs(reason: str) -> None:
    summary = {
        ...
        "final_verdict": "V5.11 输入缺少可验证的真实可用时间或版本链，未运行估值回测。",
    }
...
def self_check() -> None:
    ...
    row["mean_relative_return"] = -0.01
    assert "mean_relative_return" in failed_metrics(row)
