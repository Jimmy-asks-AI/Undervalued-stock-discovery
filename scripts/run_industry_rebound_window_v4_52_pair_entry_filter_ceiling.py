...
import run_industry_rebound_window_v4_51_entry_filter_ceiling as single


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs" / "rebound_window_v4_52_pair_entry_filter_ceiling_policy.json"
...
def main() -> None:
    ...
    source = single.load_source(config)
    grid, trades_by_id = scan_pairs(source, config)
...
def scan_pairs(source: pd.DataFrame, config: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    filters = build_filters(source, config)
    rows = []
    trades_by_id = {}
    for left, right in combinations(filters, 2):
        ...
        signal_id = f"full_sample_pair_entry_filter_{left['filter_id']}__{right['filter_id']}"
        trades["signal_id"] = signal_id
        trades["signal_name_zh"] = signal_id
        trades["signal_type"] = "full_sample_pair_entry_filter_ceiling"
        trades_by_id[signal_id] = trades
        row = stop_exit.summarize(signal_id, trades, float(config["stop_loss"]), config)
        row.update({"left_filter": left["filter_id"], "right_filter": right["filter_id"]})
        rows.append(row)
    return pd.DataFrame(rows), trades_by_id


def build_filters(source: pd.DataFrame, config: dict[str, Any]) -> list[dict[str, Any]]:
    filters = []
    for feature in config["feature_columns"]:
        ...
        for quantile in config["quantiles"]:
            ...
            filters.append({"filter_id": f"{feature}_ge_q{int(float(quantile) * 100)}", "feature": feature, "mask": values >= threshold})
            filters.append({"filter_id": f"{feature}_le_q{int(float(quantile) * 100)}", "feature": feature, "mask": values <= threshold})
    return filters
...
def choose_primary(grid: pd.DataFrame) -> dict[str, Any]:
    ...
    return frame.sort_values(["passes_core", "eligible_sample_clusters", "net_mean_return", "relative_mean_return"], ascending=[False, False, False, False]).iloc[0].to_dict()
...
def write_outputs(out: Path, debug: Path, config: dict[str, Any], source: pd.DataFrame, grid: pd.DataFrame, primary: dict[str, Any], trades: pd.DataFrame) -> None:
    summary = {
        ...
        "final_verdict": "research_only；双特征入场过滤仍不能突破收益厚度",
        "main_diagnosis": "V4.52 审计宽池固定2%止损后，现有事前双特征过滤的全样本理论上限。",
    ...
    source.to_csv(debug / "pair_entry_filter_source_panel.csv", index=False, encoding="utf-8-sig")
    grid.to_csv(debug / "pair_entry_filter_grid.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame([{"item": "full_sample_pair_filter_selection", "status": "pass", "evidence": "本版显式标记为 full_sample_pair_entry_filter_ceiling，统一评价按理论上限处理。"}]).to_csv(debug / "leakage_audit.csv", index=False, encoding="utf-8-sig")
    write_json(debug / "optimization_notes.json", {"note": "全样本双特征入场过滤上限审计，不是交易规则。"})
...
def render_report(config: dict[str, Any], row: dict[str, Any], grid: pd.DataFrame) -> str:
    ...
    return "\n".join([
        ...
        "## 解读",
        "",
        "双特征组合能制造高收益小样本，但满足 V3.1 样本和独立行情簇要求的组合仍停留在约 +1% 成本后收益附近。",
        "",
        "这说明继续在现有特征上堆简单阈值组合，不是高价值方向。",
        "",
        "## 研究边界",
        "",
