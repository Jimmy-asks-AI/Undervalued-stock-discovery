...
OUT = ROOT / "outputs" / "audit" / "rebound_leader_volume_confirmation_audit_v5_19"
DEBUG = OUT / "debug"
TOP_N = 5
CONFIRM_DAYS = 5
FILTER_COLUMN = "filter_no_severe_early_selloff"
FEATURES = ["amount_surge_rank", "early_beta_amount_score"]


def main() -> None:
    parser = argparse.ArgumentParser(description="V5.19 volume confirmation audit.")
    parser.add_argument("--self-check", action="store_true")
    args = parser.parse_args()
    if args.self_check:
        self_check()
        return
    panel = attach_amount_features()
...
def attach_amount_features() -> pd.DataFrame:
    ...
    amount_features = load_amount_features(panel[["industry_code", "confirm_date"]].drop_duplicates())
    panel = panel.merge(amount_features, on=["industry_code", "confirm_date"], how="left")
    pieces = []
    for _, event in panel.groupby(["signal_date", "entry_date", "confirm_date", "exit_date"], sort=False):
        event = event.copy()
        event["amount_surge_rank"] = event["amount_surge_5_60"].rank(pct=True)
        event["early_beta_amount_score"] = 0.70 * event["early_beta_score"] + 0.30 * event["amount_surge_rank"]
        pieces.append(event)
    return pd.concat(pieces, ignore_index=True)


def load_amount_features(keys: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for code, dates in keys.groupby("industry_code")["confirm_date"]:
        path = HISTORY_DIR / f"{str(code).zfill(6)}.csv"
        if not path.exists():
            continue
        hist = pd.read_csv(path, encoding="utf-8-sig", usecols=["日期", "成交额"])
        hist["日期"] = pd.to_datetime(hist["日期"]).dt.strftime("%Y-%m-%d")
        hist["成交额"] = pd.to_numeric(hist["成交额"], errors="coerce")
        for confirm_date in dates:
            upto = hist[hist["日期"].le(confirm_date)].tail(60)
            if len(upto) < 20:
                continue
            amount_5 = float(upto["成交额"].tail(5).mean())
            amount_60 = float(upto["成交额"].mean())
            rows.append({
                "industry_code": str(code).zfill(6),
                "confirm_date": confirm_date,
                "amount_5d": amount_5,
                "amount_60d": amount_60,
                "amount_surge_5_60": amount_5 / amount_60 if amount_60 > 0 else float("nan"),
            })
...
def evaluate(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in FEATURES:
        for keys, event in panel.groupby(["signal_date", "entry_date", "confirm_date", "exit_date"], sort=False):
...
def build_summary(results: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "version": "5.19.0",
        "policy_id": "rebound_leader_volume_confirmation_audit_v5_19",
        ...
        "best_status": "pass_volume_confirmation_gate" if passed else "research_only_no_volume_confirmation_alpha",
        "final_verdict": "V5.19 量能确认未通过完整强行业门槛，不能声称目标完成。" if not passed else "V5.19 量能确认通过强行业门槛，但仍需前推验证。",
    }
...
def write_outputs(summary: dict[str, Any], panel: pd.DataFrame, events: pd.DataFrame, results: pd.DataFrame) -> None:
    ...
    panel.to_csv(DEBUG / "volume_confirmation_opportunity_set.csv", index=False, encoding="utf-8-sig")
    events.to_csv(DEBUG / "volume_confirmation_event_panel.csv", index=False, encoding="utf-8-sig")
    results.to_csv(DEBUG / "volume_confirmation_results.csv", index=False, encoding="utf-8-sig")
...
def render_report(summary: dict[str, Any], results: pd.DataFrame) -> str:
    return "\n".join([
        ...
        "",
        "边界：V5.19 只使用确认日已知的申万二级行业成交额，测试量能放大是否改善强行业选择。",
    ])
